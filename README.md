# zipcontainer - an uncompressed zip file container library
...
This library is strict on zip file correctness. Errors during writing
can result in corrupt archives that we cannot read, but since we leave
old central directories in the archive they can easily be repaired with
an external tool such as `zip -F`.
